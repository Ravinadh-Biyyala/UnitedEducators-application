...
export function NotificationsPage() {
  ...
  const [kpiKey, setKpiKey] = useState<"unread" | "critical" | "mentions" | "approvals" | "today" | "resolved" | null>(null);
  ...
  const filtered = useMemo(() => {
    let list = [...notifs];
    if (kpiKey) {
      if (kpiKey === "unread")        list = list.filter(n => !n.read);
      else if (kpiKey === "critical") list = list.filter(n => n.severity === "critical");
      else if (kpiKey === "mentions") list = list.filter(n => n.category === "Mention");
      else if (kpiKey === "approvals")list = list.filter(n => n.category === "Approval");
      else if (kpiKey === "today")    list = list.filter(n => n.minutesAgo <= 480);
      else if (kpiKey === "resolved") list = list.filter(n => n.read);
    } else {
  ...
  }, [tab, categoryFilter, kpiKey, notifs]);
  ...
  const kpis = [
    { key: "unread"    as const, label: "Unread",    value: String(counts.unread),   sub: "Action queue",     accent: N,         icon: <Bell size={16}/>          },
    { key: "critical"  as const, label: "Critical",  value: String(counts.critical), sub: "Needs attention",  accent: "#B91C1C", icon: <AlertTriangle size={16}/> },
    { key: "mentions"  as const, label: "Mentions",  value: String(counts.mentions), sub: "You're tagged",    accent: G,         icon: <AtSign size={16}/>        },
    { key: "approvals" as const, label: "Approvals", value: String(notifs.filter(n => n.category === "Approval" && !n.read).length), sub: "Decisions pending",     accent: "#B45309", icon: <Flag size={16}/>          },
    { key: "today"     as const, label: "Today",     value: String(notifs.filter(n => n.minutesAgo <= 480).length), sub: "Last 8 hours",     accent: "#005B99", icon: <Clock size={16}/>         },
    { key: "resolved"  as const, label: "Resolved",  value: String(notifs.filter(n => n.read).length),               sub: "This week",        accent: "#15803D", icon: <CheckCheck size={16}/>    },
  ];
                ...
                <button key={t.id}
                  ...
                  {t.count > 0 && (
                    <span style={{
                      ...
                      background: t.id === "critical" ? "#B91C1C" : !kpiKey && tab === t.id ? N : "#E2E8F0",
                      color: t.id === "critical" || (!kpiKey && tab === t.id) ? "white" : TM,
