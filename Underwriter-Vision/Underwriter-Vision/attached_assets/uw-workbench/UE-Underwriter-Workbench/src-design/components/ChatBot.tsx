...
import { UnderwritingChatAssistant } from "./UnderwritingChatAssistant";
...
function SubmissionDetailView({ content, onFlagClick, onActionClick }: { content: ProactiveContent; onFlagClick: (f: Flag) => void; onActionClick: (p: string) => void }) {
  return (
    <div style={{ padding:"14px", display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {/* Clean, text + card-based AI assistant output (replaces dimensional donuts + claims bar chart) */}
      <UnderwritingChatAssistant/>
      {content.suggestedActions && (
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          {content.suggestedActions.map((a,i) => (
            <button key={i} onClick={() => a.prompt && onActionClick(a.prompt)} style={{ padding:"9px 13px", fontFamily:font, background:a.primary?`linear-gradient(135deg,${GREEN},#1F9A5A)`:"white", border:`1px solid ${a.primary?GREEN:BDL}`, borderRadius:6, color:a.primary?"white":NAVY, fontSize:"0.70rem", fontWeight:700, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:7, boxShadow:a.primary?`0 4px 12px ${GREEN}30`:SH_SM }}>
  ...
  return (
    <div style={{ width:"clamp(340px,26vw,440px)", minWidth:340, height:"100vh", background:"#F5F7FB", borderLeft:`1px solid ${BDL}`, display:"flex", flexDirection:"column", flexShrink:0, fontFamily:font, overflow:"hidden" }}>
      <div style={{ background:`linear-gradient(135deg,#0A1828,#162A4A)`, flexShrink:0, padding:"14px 16px 12px", display:"flex", alignItems:"flex-start", justifyContent:"space-between", boxShadow:"0 4px 20px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          {(mode !== "proactive" || isNewSub) && (
            <button onClick={() => { setMode("proactive"); setActiveFlag(null); if (!isNewSub) setMessages([]); if (isNewSub) { setNewSubPhase("checklist"); setReviewResult(null); } }} style={{ width:28, height:28, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:6, cursor:"pointer", transition:"background 0.2s" }}
          ...
          <button onClick={() => setIsOpen(false)} style={{ width:28, height:28, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:6, cursor:"pointer", transition:"background 0.2s" }}
            onMouseEnter={e => (e.currentTarget.style.background="rgba(255,255,255,0.15)")} onMouseLeave={e => (e.currentTarget.style.background="rgba(255,255,255,0.08)")}>
      ...
      <div style={{ flex:1, overflowY:"auto" }}>
        {/* Tab-aware suggestion panel — only rendered inside a submission detail */}
        {ctxId === "detail" && submissionWorkspace && <TabAwareSuggestions/>}
        {isNewSub && (
          <>
