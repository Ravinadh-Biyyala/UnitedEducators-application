...
import type { SubmissionProfile } from "./mockData";
...
export function adviseFactorMove(args: {
  ...
  if (newFactor > high) {
    ...
    switch (key) {
      case "GL":
        ...
        break;
      case "Property":
        context = `Brookfield's TIV of $412M with 94% sprinklered and masonry construction is a clean COPE — pricing ${above}% above book is hard to defend. Coastal CT wind exposure would be the only justification.`;
        break;
      case "ELL":
        context = `Brookfield ran zero ELL claims last 3 years. Holding ${newFactor.toFixed(2)}× would invite a competing UE-comparable carrier (Wright Specialty, Philadelphia) to undercut by 8–10%.`;
        break;
      case "Cyber":
        context = `If MFA, EDR, and offline backups are confirmed in the application, ${newFactor.toFixed(2)}× is above where I'd land Brookfield. Cyber market has softened ~12% YTD on educational risks with strong controls.`;
        break;
      case "StudentAccident":
        context = `842 day-students with no tackle football and a clean 5-yr injury history doesn't support ${newFactor.toFixed(2)}×. Risk of a participation-only quote losing the package.`;
        break;
      default:
  ...
  if (newFactor < low) {
    ...
    if (lossRatio >= 55) {
      ...
      switch (key) {
        case "Property":
          context = `Sprinkler coverage 94% and masonry construction support a credit-side rate, but going ${below}% below book is aggressive without an updated COPE survey within 12 months.`;
          break;
        case "ELL":
          ...
          break;
        default:
          context = `${below}% below book is aggressive for an account with $${(profile.member.budget || "—").toString()} operating budget — make sure this is documented in the underwriting file.`;
      }
    }
    return {
      severity: 1,
  ...
  // ── INSIDE BAND ─────────────────────────────────────────────────────
  // Vary copy by direction and how close to edge.
  ...
  // Comfortable inside band — concise narration, light chip.
  return {
    severity: 0,
    tone: "blue",
    text: dir === "up"
...
export function adviseCommissionMove(args: {
  ...
  if (delta < 0) {
    return {
      severity: 1,
      tone: "blue",
      text: `Commission to ${newComm}% — broker take drops to ${fmt$(dollarsToBroker)} (was ${fmt$(preCommissionTotal * (oldComm / 100))}). ${profile.brokerage} books ~$1.4B annually with UE; cuts below 12.5% on a sub-$200K account typically require Producer Relations sign-off. Member sees premium ${fmt$(Math.abs(memberDelta))} lighter.`,
      chips: [
