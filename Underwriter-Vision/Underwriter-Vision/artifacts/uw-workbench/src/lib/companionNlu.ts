...
export function detectIntent(raw: string): Intent {
  ...
  // Search: only on explicit verbs — "show me X", "find X", "where is X", "open X", "about X", "look up X", "search X"
  ...
  // Bare lookup ONLY when input is a single capitalized-looking token of length ≥4
  // (likely a proper noun like "Brookfield"). Anything else falls through to unknown
  // so the panel's true fallback fires.
...
// Friendly catch-all suggestion menu when we can't answer directly.
