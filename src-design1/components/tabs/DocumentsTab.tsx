...
export function DocumentsTab() {
  ...
  const react = useTabCompanion("documents");
  ...
  const openPreview  = (doc: Doc) => {
    setPreviewDoc(doc);
    react(
      `Opened **${doc.name}** (${doc.category}). I'll watch your highlights — pin any that affect rating or referral.`,
      [
        { id: "extract-loss",  label: "Extract loss data",     tone: "blue",  icon: "search" },
        { id: "compare-prior", label: "Compare to prior year", tone: "gold",  icon: "diff"   },
        { id: "request-more",  label: "Request missing docs",  tone: "green", icon: "mail"   },
      ],
    );
  };
                ...
                {folderStats.map((f) => {
                  ...
                  return (
                    <button
                      key={f.id}
                      onClick={() => {
                        if (selectedFolder === f.id) return;
                        setSelectedFolder(f.id);
                        setSearch("");
                        const missing = f.missing > 0 ? ` ${f.missing} missing.` : "";
                        const inRev = f.inReview > 0 ? ` ${f.inReview} in review.` : "";
                        react(`Switched to **${f.label}** folder. ${f.uploaded}/${f.total} uploaded.${missing}${inRev}`);
                      }}
                      onKeyDown={(e) => {
