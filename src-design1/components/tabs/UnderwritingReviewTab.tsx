...
export function UnderwritingReviewTab() {
  ...
  const react = useTabCompanion("review");

  const setDecision = (id: string, d: Decision) => {
    setDecisions(prev => ({ ...prev, [id]: d }));
    const item = REVIEW_DATA.find(r => r.id === id);
    if (!item) return;
    ...
    const advice =
      d === "pass"  ? "Looks clean. I'll mark this complete on the review checklist." :
      d === "flag"  ? "Flagging this drives a referral note — I'll draft a subjectivity for you to review under Approvals." :
      d === "waive" ? "Waiving is logged in the audit trail with your name and timestamp. SR-UW will see this on review." :
                       "Reset to pending — no audit entry yet.";

    react(`Marked **${item.title}** as ${label}.\n\n${advice}`, [
      { id: "open-next",    label: "Open next item",      tone: "blue",  icon: "arrow-right" },
      { id: "add-task",     label: "Add follow-up task",  tone: "gold",  icon: "check"       },
      { id: "view-audit",   label: "Show audit entry",    tone: "green", icon: "shield"      },
    ]);
  };
  ...
  if (selected) {
    return (
      <ReviewDetail
        ...
        onDecide={(d) => setDecision(selected.id, d)}
      />
    );
  }
        ...
        {REVIEW_DATA.map((r, idx) => {
          ...
          return (
            <ChecklistRow
              ...
              onOpen={() => onOpen(r.id)}
              onDecide={(next) => onDecide(r.id, next)}
            />
          );
        })}
      </div>
...
function ChecklistRow({
  item, idx, decision, isLast, onOpen, onDecide,
}: {
  item: ReviewItem;
  idx: number;
  decision: Decision;
    ...
    <div
      ...
      <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
        ...
        <ActionButton tone="refer"   active={decision === "referred"}
          onClick={() => onDecide(decision === "referred" ? "pending" : "referred")}/>
        <button
          onClick={onOpen}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                style={{ fontSize: "0.84rem", border: `1px solid #C4CDD8`, color: TD, fontFamily: font, background: "white", borderRadius: 6 }}
              />
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
