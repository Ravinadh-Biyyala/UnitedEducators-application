...
export function NotesTab() {
  ...
  const handleAddNote = () => {
    ...
    setShowModal(false);
    const tagStr = newEntry.tags.length ? ` Tagged: ${newEntry.tags.map(t => `#${t}`).join(" ")}.` : "";
    react(
      `Saved your note **${newEntry.id}**.${tagStr}\n\nI'll surface it in the readout if it relates to a referral or audit question.`,
      [
        { id: "pin-note",   label: "Pin this note",    tone: "gold",  icon: "pin"  },
        { id: "tag-note",   label: "Add a tag",        tone: "blue",  icon: "tag"  },
        { id: "summarize",  label: "Summarize notes",  tone: "green", icon: "list" },
      ],
    );
  };
            ...
            {([
              ...
              return (
                <button key={f.key} onClick={() => {
                    ...
                    setFilter(f.key);
                    const blurb =
                      f.key === "All"    ? `Showing all ${f.count} notes on this submission.` :
                      f.key === "Mine"   ? `Filtered to your ${f.count} note(s). Useful when prepping a hand-off.` :
                                            `Filtered to ${f.count} pinned note(s) — these are the ones flagged as material for the file.`;
                    react(blurb);
                  }}
