...
import { useTabCompanion } from "../companion/useTabCompanion";
...
export function NotesTasksTab() {
  ...
  const react = useTabCompanion("notes");

  const handleNoteTypeChange = (t: Note["type"]) => {
    if (t === noteType) return;
    setNoteType(t);
    const hint = t === "Broker Communication"
      ? "Broker comms are auto-stamped and visible to the broker portal — keep it factual."
      : t === "Internal"
      ? "Internal notes are not shared with the broker. Good for back-channel UW reasoning."
      : "UW notes are part of the audit file and visible to SR-UW + Lead on referral.";
    react(`Note type switched to **${t}**. ${hint}`);
  };

  const handleAddNoteNT = () => {
    if (!newNote.trim()) return;
    const blurb = `Note saved as **${noteType}** (${newNote.trim().length} chars). ${referralFlagged ? "Referral flag is on — SR-UW will be notified." : ""}`;
    setNewNote("");
    react(blurb, [
      { id: "flag-ref",  label: "Flag for referral",  tone: "red",   icon: "flag" },
      { id: "summarize", label: "Summarize this file", tone: "gold",  icon: "list" },
      { id: "open-task", label: "Spawn a task",        tone: "green", icon: "check" },
    ]);
  };

  const handleReferralToggle = () => {
    const next = !referralFlagged;
    setReferralFlagged(next);
    react(next
      ? `Submission flagged for **SR-UW referral**. I've drafted a referral note — review under the Approvals tab when ready.`
      : `Referral flag cleared. The submission is back in your queue.`,
    );
  };
          ...
          <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BDL}` }}>
            ...
            <div className="flex items-center gap-1.5 mb-3">
              {(["UW Note", "Internal", "Broker Communication"] as Note["type"][]).map(t => (
                <button key={t} onClick={() => handleNoteTypeChange(t)}
