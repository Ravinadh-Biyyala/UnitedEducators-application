...
export function ApprovalsTab() {
  ...
  const react = useTabCompanion("approvals");
  ...
  const handleApprove = (id: string) => {
    ...
    if (item) {
      const stillPending = approvals.filter(a => a.id !== id && a.status === "Pending").length;
      react(
        `Approved **${item.id} — ${item.subject}** on ${now}.\n\n${stillPending === 0 ? "No pending approvals remain — this submission is clear to bind once Rating is finalized." : `${stillPending} approval${stillPending === 1 ? "" : "s"} still pending.`}`,
        [
          { id: "notify-broker",  label: "Notify broker",       tone: "blue",  icon: "mail" },
          { id: "open-quote",     label: "Open Rating",         tone: "gold",  icon: "calculator" },
          { id: "audit",          label: "Show audit entry",    tone: "green", icon: "shield" },
        ],
      );
    }
  };
  ...
  const handleDecline = (id: string) => {
    ...
    if (item) {
      react(
        `Declined **${item.id} — ${item.subject}**. I've flagged the submission for re-review.\n\nWant me to draft a decline note to the broker?`,
        [
          { id: "draft-decline", label: "Draft decline note", tone: "red",   icon: "mail"  },
          { id: "request-info",  label: "Request more info",  tone: "blue",  icon: "info"  },
          { id: "audit",         label: "Show audit entry",   tone: "green", icon: "shield" },
        ],
      );
    }
  ...
  const handleAdd = (partial: Omit<Approval, "id">) => {
    const id = `AP-${nextNum.current++}`;
    setApprovals(prev => [{ ...partial, id }, ...prev]);
    react(`New approval request **${id} — ${partial.subject}** sent to ${partial.approver}. SLA: 24h. I'll alert you if it stalls.`);
  };
