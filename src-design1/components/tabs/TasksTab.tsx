...
export function TasksTab() {
  ...
  const react = useTabCompanion("tasks");

  const handleAddTask = (partial: Omit<Task, "id" | "status" | "completedAt">) => {
    setTasks(prev => [{ ...partial, id: nextId.current++, status: "Open" }, ...prev]);
    react(
      `New task created: **${partial.title}** → ${partial.assignee}. Due ${partial.due}.\n\nI'll watch the SLA timer and ping you 24h before it goes overdue.`,
      [
        { id: "assign-self", label: "Reassign to me",    tone: "blue",  icon: "user" },
        { id: "set-prio",    label: "Bump priority",     tone: "gold",  icon: "flag" },
        { id: "view-queue",  label: "Open task queue",   tone: "green", icon: "list" },
      ],
    );
  };
  ...
  const handleComplete = (id: number) => {
    ...
    if (task) {
      const remaining = tasks.filter(t => t.id !== id && t.status !== "Done").length;
      react(`Closed **${task.title}**. ${remaining === 0 ? "All open tasks cleared on this submission — you're good to advance." : `${remaining} open task${remaining === 1 ? "" : "s"} remaining.`}`);
    }
  };
  ...
  const handleReopen = (id: number) => {
    ...
    if (task) react(`Reopened **${task.title}**. I'll add it back to your active queue.`);
  };
