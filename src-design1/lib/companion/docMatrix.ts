...
// Quick helper used by the SubmissionDetail companion: given a profile-style
// document list (status: Verified | Pending | Missing) and a current stage,
// return human-readable strings for what's outstanding before quote-ready.
export function missingDocsSummary(
  documents: { name: string; status: "Verified" | "Pending" | "Missing" }[],
): { missing: string[]; pending: string[] } {
  return {
    missing: documents.filter(d => d.status === "Missing").map(d => d.name),
    pending: documents.filter(d => d.status === "Pending").map(d => d.name),
  };
}
