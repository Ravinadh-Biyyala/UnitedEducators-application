...
function KPITile({ k }: { k: { label: string; value: string; sub: string; color: string; icon: React.ReactNode } }) {
    ...
    <div
      ...
      style={{
        background: hovered
          ...
          : "white",
        border: `1px solid ${hovered ? `${k.color}40` : BDL}`,
        borderRadius: 10,
        padding: "14px 16px",
        boxShadow: hovered
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="inline-flex items-center justify-center"
          style={{
            ...
            background: hovered ? `${k.color}1F` : `${k.color}10`,
            color: k.color,
  ...
  return (
    <AppShell activePage="portfolio" role={role} onRoleChange={() => {}}>
      <div className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 space-y-5 sm:space-y-6"
        ...
        <PageRegister
          ...
          respond={(sid) => {
            ...
            if (sid === "monthly") return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
              ...
              ctaLabel: "Add to Portfolio page",
              dashboardHref: "/portfolio",
            } }];
            if (sid === "lr") {
          ...
          freeText={(text) => {
            ...
            // Monthly / premium trend → linecard with Add-to-Portfolio CTA
            ...
            // Loss ratio trend
            if (/\b(loss ratio trend|lr trend|loss-ratio trend)\b/.test(t)) {
              const lrTrend = PREMIUM_TREND.map((p, i) => ({ label: p.month, value: 48 + Math.round(Math.sin(i) * 6) + i }));
              return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
                kind: "linecard",
                title: "Loss ratio trend",
            ...
            // Bind rate
            if (/\b(bind rate|conversion|close rate)\b/.test(t)) {
              const bindTrend = PREMIUM_TREND.map((p, i) => ({ label: p.month, value: 28 + Math.round(Math.cos(i) * 5) + Math.floor(i / 2) }));
              return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
                kind: "linecard",
                title: "Bind rate by month",
            ...
            if (/\b(by segment|segment (mix|breakdown|cut)|premium mix)\b/.test(t)) {
              return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
                kind: "donut", title: "Written premium by segment",
                segments: SEGMENT_DATA.map(s => ({ label: s.name, value: s.premium, color: s.color })),
              } }];
            ...
            // Account count / size summary
            if (/\b(how many accounts|total accounts|book size|written premium total)\b/.test(t)) {
              const accts = SEGMENT_DATA.reduce((s, x) => s + x.accounts, 0);
              const prem = SEGMENT_DATA.reduce((s, x) => s + x.premium, 0).toFixed(1);
              return [{ id: newId(), role: "agent", kind: "text", ts: now(),
                text: `Book size: ${accts} accounts · $${prem}M written premium across ${SEGMENT_DATA.length} segments.` }];
            }

            // Geographic
            if (/\b(geo|geography|state|states|region)\b/.test(t)) {
              return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
                kind: "bars", title: "Premium by state",
                unit: "$M",
        ...
        {/* Pinned charts (added from Companion) */}
        <PinnedDashboard />
