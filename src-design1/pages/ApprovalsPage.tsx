...
export function ApprovalsPage() {
  ...
  const kpis = [
    { label: "Awaiting Decision", value: String(counts.awaiting),       sub: fmt$(pendingAmount),  accent: "#8A5C00", icon: <Clock size={16}/>         },
    { label: "SLA Breach (24h+)", value: String(slaBreached),           sub: "Pending > 1 day",     accent: "#B91C1C", icon: <AlertTriangle size={16}/> },
    { label: "Escalated",         value: String(counts.escalated),      sub: "Director queue",      accent: N,         icon: <Flag size={16}/>          },
    { label: "Approved (MTD)",    value: String(approvedToday),         sub: "This month",          accent: "#15803D", icon: <CheckCircle2 size={16}/>  },
    { label: "Declined (MTD)",    value: String(decliedToday),          sub: "This month",          accent: "#B91C1C", icon: <XCircle size={16}/>       },
    { label: "Avg Decision Time", value: "18h",                         sub: "Last 30d",            accent: G,         icon: <ShieldCheck size={16}/>   },
  ];
  ...
  return (
    <AppShell activePage="approvals" role={role} onRoleChange={() => {}}>
      <div className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 space-y-5 sm:space-y-6"
        ...
        <PageRegister
          routeKey="page:approvals"
          title="Approvals"
          subtitle="Authority referrals · SLA tracking"
          greeting="Approvals queue: 4 pending, 2 in your tray, 0 escalations. Want me to triage the queue or draft a justification for the next one?"
          suggestions={[
            { id: "pending", label: "Pending approvals", tone: "blue",   icon: "Clock" },
            { id: "mine",    label: "My approval queue", tone: "violet", icon: "ShieldCheck" },
            { id: "sla",     label: "Approval SLA status", tone: "gold", icon: "Activity" },
            { id: "blockers", label: "What's blocking approval?", tone: "red", icon: "AlertTriangle" },
          ]}
          respond={(sid) => {
            if (sid === "pending") return [{ id: newId(), role: "agent", kind: "text", ts: now(),
              text: "Pending approvals (4):\n• SUB-7829 — 3-yr rate-lock authority (Manager)\n• SUB-7831 — Premium $200K+ (Director)\n• SUB-7836 — Reinsurance SAM $5M (Re team)\n• SUB-7842 — Bind authority over $250K (Manager)\n\nLikely cleared today: SUB-7829 and SUB-7842." }];
            if (sid === "mine") return [{ id: newId(), role: "agent", kind: "text", ts: now(),
              text: "Your approval queue (2):\n• SUB-7831 — Authority referral, 18h pending\n• SUB-7836 — Reinsurance check, 1d pending\n\nBoth within SLA. No escalations needed." }];
            if (sid === "sla") return [{ id: newId(), role: "agent", kind: "text", ts: now(),
              text: "Approval SLA — avg 1.8d (target ≤2d) · 91% within-SLA · 0 currently at risk · 1 breached this month. Team trending green." }];
            if (sid === "blockers") return [{ id: newId(), role: "agent", kind: "text", ts: now(),
              text: "Blocking approval right now:\n1. SAM $5M reinsurance referral — awaiting actuarial sign-off (1d remaining)\n2. Title IX investigation log — required before Governance review can close\n\nNo other blockers in the chain." }];
          }}
        />
                ...
                <button key={t.id}
                  ...
                  {t.count > 0 && (
                    <span style={{
                      ...
                      background: t.id === "escalated" ? "#B91C1C" : tab === t.id ? N : "#E2E8F0",
                      color: t.id === "escalated" || tab === t.id ? "white" : TM,
