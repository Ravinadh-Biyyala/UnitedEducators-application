...
export function TaskQueuePage() {
  ...
  const kpis = [
    { label: "My Open Tasks",    value: String(ALL_TASKS.filter(t => t.assignee === myName && t.status !== "Done").length), sub: "Active",          accent: N,         icon: <CheckSquare size={16}/>       },
    { label: "Overdue",          value: String(ALL_TASKS.filter(t => t.status === "Overdue").length),                       sub: "Past due",        accent: "#B91C1C", icon: <AlertCircle size={16}/>       },
    { label: "Due Today",        value: String(ALL_TASKS.filter(t => t.daysUntilDue === 0 && t.status !== "Done").length),  sub: "Today",           accent: "#B45309", icon: <AlertTriangle size={16}/>     },
    { label: "In Progress",      value: String(ALL_TASKS.filter(t => t.status === "In Progress").length),                   sub: "Active work",     accent: "#005B99", icon: <Clock size={16}/>             },
    { label: "Completed (MTD)",  value: String(ALL_TASKS.filter(t => t.status === "Done").length),                          sub: "This month",      accent: "#15803D", icon: <CheckCircle2 size={16}/>      },
    { label: "SLA at Risk",      value: String(ALL_TASKS.filter(t => t.slaUsedPct >= 80 && t.status !== "Done").length),    sub: "≥80% SLA used",   accent: G,         icon: <SlidersHorizontal size={16}/> },
  ];
          ...
          respond={(sid) => {
            if (sid === "overdue") {
              ...
              return [{ id: newId(), role: "agent", kind: "viz", ts: now(), viz: {
                ...
                items: od.slice(0, 5).map(t => ({ ok: false, label: t.title, sub: `${t.member} · due ${t.due}` })),
              } }];
            }
            if (sid === "critical") {
                ...
                <button key={t.id}
                  ...
                  {t.count > 0 && (
                    <span style={{
                      ...
                      background: t.id === "overdue" ? "#B91C1C" : tab === t.id ? N : "#E2E8F0",
                      color: t.id === "overdue" || tab === t.id ? "white" : TM,
